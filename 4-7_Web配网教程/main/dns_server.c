...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len)
{
    ...
    // Respond to all questions with the ESP32's IP address 用ESP32的IP地址回答所有问题
    for (int i = 0; i < qd_count; i++)
    {
        char *name_end_ptr = parse_dns_name(cur_qd_ptr, name, sizeof(name));
        if (name_end_ptr == NULL)
        {
            ESP_LOGE(TAG, "Failed to parse DNS question: %s", cur_qd_ptr); // 无法解析dns问题
            return -1;
        }
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                if (reply_len <= 0)
                {
                    ESP_LOGE(TAG, "Failed to prepare a DNS reply"); // 无法准备DNS回复
                }
                else
                {
